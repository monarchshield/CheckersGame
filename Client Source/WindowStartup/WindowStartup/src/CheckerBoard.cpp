...
bool CheckerBoardTile::MadatoryMove()
{
	...
	{
		if (m_Directions[0]->Name == "FORWARD_LEFT" && m_LCheck || m_Directions[0]->Name == "FORWARD_RIGHT" && m_FRight  ||
			m_Directions[0]->Name == "BACK_LEFT" && m_Bleft || m_Directions[0]->Name == "BACK_RIGHT" && m_Bright) 
		{
			TileSelected = true; 
			return true;
		}
...
void CheckerBoardTile::ValidMoves(std::vector<CheckerBoardTile *> Tiles)
{
	...
	//You can jump! :)
	if (ForwardLeft != ObjRefrence && ForwardLeft + 7 < 64 && TempTiles[ForwardLeft]->isOccupied()  && m_RowRef != 1 && checker != nullptr &&  checker->GetColour() != TempTiles[ForwardLeft]->checker->GetColour())
	{
		ForwardLeft += 7;
		m_LCheck = true;
...
CheckerPiece* CheckerBoardTile::DeletePiece()
{
	...
	if (m_Directions[KillNumber]->Name == "FORWARD_LEFT" && m_LCheck && ObjRefrence + 7 < 64)
